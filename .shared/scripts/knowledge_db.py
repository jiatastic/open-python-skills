#!/usr/bin/env python3
"""
Local Knowledge Database for Python Backend Pro Max skill.
...
def format_results(results: list[SearchResult], verbose: bool = False) -> str:
    ...
    for i, r in enumerate(results, 1):
        ...
        output.append(f"    Category: {r.category} | Relevance: {r.relevance:.0%} | Code: {'✓' if r.has_code else '✗'}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Search Python Backend Pro Max knowledge database",
  ...
  %(prog)s --get project-structure
    ...
    if args.tag and not args.query:
        ...
        if entries:
            ...
            for e in entries:
                code_mark = "✓" if e.get("code_examples") else "✗"
